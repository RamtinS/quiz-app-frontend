...
describe('User stats page test', () => {
  beforeEach(() => {
    ...
    cy.login('http://localhost:3000/login?redirect=/user-stats', 'TestUser', 'password');
  });
  ...
  it('displays error message on data fetch failure', () => {
    ...
    cy.intercept('GET', 'http://localhost:8080/api/v1/users/stats', { statusCode: 500 });

    cy.get('[data-cy="error-message"]').should('exist');

    cy.get('[data-cy="error-message"]').should('contain',
