...
const currentQuestion = computed(
  () => createdQuestions.value[questionIndex.value],
);
 ...
 */
const handleAddQuestionEmit = (emitData: {
...
async function postQuizToServer() {
  ...
  for (let i = 0; i < createdQuestions.value.length; i++) {
    ...
    if (question.questionType === "MultipleChoiceQuestionDTO") {
      const message = QuizValidationUtility.multipleChoiceQuestionIsValid(
        question as MultipleChoiceQuestionDTO,
      );
      if (message) {
      ...
      const message = QuizValidationUtility.trueOrFalseQuestionIsValid(
        question as TrueOrFalseQuestionDTO,
      );
      if (message) {
        quizErrorMessage.value = "Question " + (i + 1) + ": " + message;
        return;
      ...
      quizErrorMessage.value =
        "Question number " + (i + 1) + " has an unknown question type";
      return;
    }
  }
  ...
  try {
    if (props.preExistingQuiz) {
      await QuizCreationService.updateQuizForLoggedInUser(
        quizCreationRequestDTO,
        props.preExistingQuiz.quizId,
      );
    } else {
      await QuizCreationService.postQuizForLoggedInnUser(
        quizCreationRequestDTO,
      );
    }
...
async function deleteQuiz() {
  if (props.preExistingQuiz) {
    try {
      ...
      quizErrorMessage.value = await ErrorHandlingService.handleRequestError(
        err,
        "Deletion failed",
      );
    }
...
<template>
  <div
    id="quiz-creator"
    v-if="!quizHasBeenSubmitted"
    @click="quizErrorMessage = ''"
  >
    <div id="quiz-info">
      <h1 v-if="quizSettings.title">
        {{ quizSettings.title }}
      </h1>
      <h1 v-else-if="props.preExistingQuiz">Edit quiz</h1>
      <h1 v-else>Create a quiz</h1>
      <div class="button-container">
        <button
          v-if="props.preExistingQuiz"
          class="delete-button"
          @click="deleteQuiz"
        >
          Delete quiz
        ...
        <button v-if="!quizErrorMessage" @click.stop="postQuizToServer">
          Submit quiz
        </button>
        <p v-if="quizErrorMessage" class="post-quiz-error-message">
          {{ quizErrorMessage }}
        </p>
      </div>

      <QuestionSettingsModal
        v-show="showQuizSettingsModal"
    ...
    <div id="previews" class="preview-container">
      <button class="add-question-button" @click="showQuestionTypeModal = true">
  ...
  <div v-else class="confirmation-message">
    <p v-if="props.preExistingQuiz">Quiz has been edited, you may return</p>
    <p v-else>Quiz has been submitted, you may return</p>
    <button>
