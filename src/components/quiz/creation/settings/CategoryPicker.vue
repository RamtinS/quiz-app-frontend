...
async function fetchPossibleCategories(page: number) {
  try {
    const categories = await CategoryService.getCategoriesPaginated(
      page,
      pageSize,
    );
...
<template>
  ...
  <div class="category-picker">
    <button @click="toggleDropdown">Select categories</button>
    <div v-if="showDropDown" class="dropdown">
      ...
      <div>
        <button @click="nextPage">Load more categories</button>
      </div>
      <div>
  ...
  <div id="debug" v-show="false">Selected: {{ selectedCategory }}</div>
</template>
